...
def build_route_result(
    ...
    path = [edges[0].source] + [edge.target for edge in edges]
    ...
    return RouteResult(
        ...
        total_emissions=sum(e.emissions for e in edges),
        total_distance_km=sum(e.distance_km for e in edges),
        score=route_score_from_edges(edges, weights, scales),
        weights=weights,
    )


def astar_multimodal_with_forbidden_arcs(
    graph: Dict[str, List[Edge]],
    start: str,
    goal: str,
    weights: Dict[str, float],
    ...
    edges.reverse()
    return build_route_result(str(weights["name"]), edges, weights, scales)


def generate_tabu_neighbors(
    ...
    for edge in candidate_edges[:max_neighbors]:
        ...
        candidate = improve_route_by_shortcuts(candidate, graph, weights, scales)
        key = tuple(candidate.path)

        if key == tuple(current_route.path) or key in seen_paths:
            continue
...
def tabu_search_route(
    ...
    # Startloesung mit demselben begrenzten Verzweigungsgrad wie das schnelle
    # Dijkstra-Skript -- dadurch ist sie haeufig suboptimal. Die folgende
    # Tabu-Nachbarschaftssuche arbeitet danach ohne dieses Limit und kann die
    # Startloesung dadurch tatsaechlich verbessern.
    ...
    if current is None:
        ...
        # Damit ueberhaupt eine Startloesung existiert, hier einmal ohne
    ...
    for iteration in range(1, max_iterations + 1):
        neighbors = generate_tabu_neighbors(
            ...
            scales=scales,
            max_expansions=max_expansions,
            max_neighbors=neighbors_per_iteration,
        )
    ...
    best.route_type = str(weights["name"]) + " + Tabu Search"
    return best


def calculate_four_tabu_routes(
    graph: Dict[str, List[Edge]],
    start: str,
    goal: str,
    max_expansions: int,
    ...
    for weights in weight_sets:
        result = tabu_search_route(
            ...
            no_improvement_limit=int(USER_INPUT["tabu_no_improvement_limit"]),
        )
...
def main() -> None:
    ...
    routes = calculate_four_tabu_routes(
        graph=graph,
        start=start,
        goal=end,
        max_expansions=max_expansions,
