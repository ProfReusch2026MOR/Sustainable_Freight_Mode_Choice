...
Network Flow Planning (NFP)-Probleme gehen von einer feste, unveränderlichen Infrastruktur aus, bei der alle Kantenkapazitäten und angebotenen Dienste (wie feste Fahrpläne) bereits vorgegeben sind. Die planerische Entscheidung beschränkt sich in diesem Fall ausschließlich darauf, wie viel Fluss (z. B. Sendungsmenge) über welche der existierenden Pfade geleitet wird. Ziel der Optimierung ist es, die rein variablen Transportkosten zu minimieren, während Restriktionen der Infrastruktur eingehalten werden.
...
Das mathematische Fundament für Service-Network-Design-Probleme bildet das Fixed-Cost Capacitated Multicommodity Network Design (CMND). Wie in @ghamlouche2003cycle beschrieben, repräsentiert das CMND-Modell eine generische Formulierung für eine Vielzahl von Planungs- und Betriebsaufgaben in Transport- und Logistiksystemen.
